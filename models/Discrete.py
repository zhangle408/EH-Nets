...
class DiscreteFunction(Function):
    ...
    @staticmethod
    def forward(ctx, aux_combination, balance_weight, avg_freq, n_sc):
        ...
        n_sc=torch.LongTensor([n_sc]).to(aux_combination.device)
        ctx.save_for_backward(aux_combination, balance_weight, avg_freq, n_sc)
    ...
    def backward(ctx, grad_output):
        ...
        n_sc =int(ctx.saved_tensors[3].view(-1).cpu().numpy())
