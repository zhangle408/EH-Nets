...
def select_filter(n_lego, drop_filter_stage, last_rate, fre_num, lego_filter):
    ...
    mask=torch.FloatTensor(np.ones((fre_num, n_lego,1,1,1), dtype=np.float32)).to(lego_filter.device)
    lego_filter_detach=lego_filter.detach()
    entropy_list=entropy_filter(lego_filter_detach)
    for i in range(fre_num):
        drop_filter=int((1.-preserve_rate[i])*n_lego)
        if drop_filter>0:
            topk_maxmum, _ = entropy_list.topk(int((1.-preserve_rate[i])*n_lego), dim=0, largest=False, sorted=False)
...
class SCFConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bais, kernel_size,  n_lego, fre_num, last_rate, balance_weight):
        ...
        self.lego = nn.Parameter(nn.init.kaiming_normal_(torch.rand(self.n_lego, self.basic_channels, self.kernel_size, self.kernel_size),mode='fan_out', nonlinearity='relu'))#m,N,3,3
        self.aux_coefficients = nn.Parameter(init.kaiming_normal_(torch.rand(self.fre_num, self.out_channels, self.n_lego, 1, 1)))#K^2,M,m,1,1
        self.aux_combination = nn.Parameter(init.kaiming_normal_(torch.rand(self.fre_num, self.out_channels, self.n_lego, 1, 1)))
